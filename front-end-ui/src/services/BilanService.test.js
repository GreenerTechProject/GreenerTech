...
describe('BillonService', () => {
  it('should fetch bilna positions successfully', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(`${window.location.protocol}//${window.location.hostname}:8080/api/bilan/list`);
  });
  ...
  it('should save bilan position successfully', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(`${window.location.protocol}//${window.location.hostname}:8080/api/bilan/save`, mockPosition);
  });
  ...
  it('should delete bilan on maps', async () => {
    ...
    expect(response).toEqual({ success: true });
    expect(axios.delete).toHaveBeenCalledWith(`${window.location.protocol}//${window.location.hostname}:8080/api/bilan/delete`, mockBillon);
  });
});
